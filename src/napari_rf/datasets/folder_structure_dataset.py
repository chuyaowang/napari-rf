        ...
        for p, dirnames, filenames in os.walk(self.root):
            if filenames:
                for f in filenames:
                    ...
                    if not 'BACKGR' in path.upper():
                        if f.upper().endswith('TIF') or f.upper().endswith('TIFF'):
                            paths.append(path)

        temp = []
        for path in paths:
            match = re.findall(f'channel_(\d+)', path)
            if match:
                channel = int(match[0])
            else:
                channel = None

            match = re.findall(f'frame_(\d+)', path)
            if match:
                frame = int(match[0])
            else:
                frame = None

            match = re.findall(f'z_level_(\d+)', path)
            if match:
                z = int(match[0])
            else:
                z = None

            match1 = re.findall(f'position_(\d+)', path)
            match2 = re.findall(f'pos_(\d+)', path)
            if match1:
            ...
            if (not self.channels) | (channel is None) | (channel in self.channels):
                if (not self.frames) | (frame is None) | (frame in self.frames):
                    if (not self.positions) | (position is None) | (position in self.positions):
                        if (not self.z_levels) | (z is None) | (z in self.z_levels):
                            if z is None:
                                z = 0
                            if position is None:
                                position = 0
                            ...
                            temp.append([path, f'position_{position}/z_level_{z}/channel_{channel}/frame_{frame}.tif'])
